...
JSON_DATA = {
    "operationName": "SizeSelector",
    "variables": {
        "productId": "aafa5e04-6509-4ff2-90aa-8742ec9fd6d5",
        "countryCode": "CN",
        "currencyCode": "USD",
        "marketName": ""
    },
    "query": "query SizeSelector($productId: String!, $countryCode: String!, $currencyCode: CurrencyCode, $marketName: String) { product(id: $productId) { id title brand primaryCategory productCategory sizeAllDescriptor sizeDescriptor defaultSizeConversion { name type } availableSizeConversions { name type } market(currencyCode: $currencyCode) { currencyCode state(country: $countryCode) { askServiceLevels { expressExpedited { count lowest { amount } } expressStandard { count lowest { amount } } standard { count lowest { amount } } } lowestAsk { amount } highestBid { amount } } } listingType variants { id hidden group { id shortCode } sortOrder traits { size } sizeChart { displayOptions { size type } } market(currencyCode: $currencyCode) { state(country: $countryCode, market: $marketName) { askServiceLevels { expressExpedited { count lowest { amount } } expressStandard { count lowest { amount } } standard { count lowest { amount } } } lowestAsk { amount } highestBid { amount } } } } } }"
}

if __name__ == "__main__":
    print("🚀 Sending FRESH request to StockX...")
    try:
        resp = requests.post(
            "https://gateway.stockx.com/api/graphql",
            headers=HEADERS,
            json=JSON_DATA,
            # impersonate="chrome120",  # 最接近 Android 的指纹
            timeout=10
        )
