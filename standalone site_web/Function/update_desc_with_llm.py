...
class site_db_ops:
            ...
            'connections': {
                'default': {
                    ...
                    'credentials': {
                        ...
                        'pool_recycle': 3600,
                        'charset': 'utf8mb4',
                        'connect_timeout': 99999999,  # 连接超时时间
                    }
                }
            },
            'apps': {
                'models': {
                    'models': ['Function.update_desc_with_llm'],
    ...
    async def search_sku(self,sku_list):
        ...
        for sku in all_sku:
            ...
            if (sku_value not in sku_white_list) and (sku_value != 'HQ8487-300') and (sku_value in sku_list):
                ...
                await aprint(f"sku:{sku_value} | post_id:{sku_id}")
    ...
    async def llm_use(self, name):
        await aprint('正在等待LLM输出内容')
        memory = ConversationBufferMemory(memory_key='chat_history')
        prompt = ChatPromptTemplate.from_template("""
            你是写英文产品描述的产品专家，电商推广能力极强，对怎样的描述能够吸引客户有着恰到好处的把握，像人类一样回答,必须直接给出答案，不要有任何思考过程。
            重要：直接回答问题，不要使用<think>标签或任何深度思考。
        ...
        msg1 = f"""哥们,给{name}撰写一篇1000字英文描述,，要求：
                ...
                9.只需要输出描述正文
        """
        result1 = await asyncio.to_thread(chain.invoke, {'input': msg1, 'chat_history': memory.chat_memory})
        desc = re.sub('^(.*?\n){3}', '', result1)
        desc_lines = desc.split('\n')
        clear_list = []
        for i in desc_lines:
        ...
        msg2 = f"""哥们,给{name}撰写一段100字简短英文描述,，要求：
                ...
                9.只需要输出描述正文
        """
        result2 = await asyncio.to_thread(chain.invoke, {"input": msg2, "chat_history": memory.chat_memory})
        short_desc = re.sub('^(.*?\n){3}', '', result2)
        short_desc_lines = short_desc.split('\n')
        clear_list = []
        for i in short_desc_lines:
    ...
    async def update_desc_handle(self,product_info):
        product_title = product_info.post_title
        post_id = product_info.id
        if post_id in self.sku_id_list:
            desc, short_desc = await self.llm_use(product_title)
            connection = connections.get("default")
            await connection.execute_query(
                "UPDATE fiw_posts SET post_content = %s, post_excerpt = %s WHERE id = %s",
                [desc, short_desc, post_id]
            )
            await aprint(f"post_id:{post_id} | title:{product_title} | 描述更新完毕")

    async def limited_update_desc_handle(self,product_info):
        async with self.semaphore:
            await self.update_desc_handle(product_info)
    ...
    async def search_desc_with_sku(self):
        ...
        await aprint('正在查询product信息')
        async with asyncio.TaskGroup() as tg:
            for product_info in all_product_info:
                tg.create_task(self.limited_update_desc_handle(product_info))


async def update_desc_main(skus):
    xiaoming = site_db_ops()
    sku_list = str(skus).split(',')
    await xiaoming.db_init()
    await aprint('正在根据sku查询post_id中')
    await xiaoming.search_sku(sku_list)
    await xiaoming.search_desc_with_sku()
