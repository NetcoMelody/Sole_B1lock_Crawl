        ...
        async with aiohttp.ClientSession() as session:
            async with TaskGroup() as tg:
                if os.path.exists('need_update.csv'):
                    async with aiofiles.open('need_update.csv', mode='r', encoding='utf-8-sig') as f:
                        reader = aiocsv.AsyncDictReader(f)
                        await self.update_if_handle(reader,session,tg)


                if os.path.exists('matched_update.csv'):
                    async with aiofiles.open('matched_update.csv', mode='r', encoding='utf-8-sig') as f:
                        reader = aiocsv.AsyncDictReader(f)
                        await self.update_if_handle(reader, session, tg)
...
async def main():
    ...
    os.remove('raw.csv')
    os.remove('sku.txt')
    # if os.path.exists('get_api_data.csv'):
...
if __name__ == '__main__':
    uvicorn.run('fusion_proces_v3_Beta:app', host='192.168.0.18', port=8080)
